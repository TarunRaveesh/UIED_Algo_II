...
def main():
    ...
    trainloader = DataLoader(
        trainset, batchsize, shuffle=True, num_workers=n_workers)
...
def save_checkpoint(state, is_best):
    ...
    if epoch % freq == 0:
        shutil.copyfile(
            filename, './checkpoints/model_{}.pth.tar'.format(epoch))
    if is_best:
        shutil.copyfile(
            filename, './checkpoints/model_best_{}.pth.tar'.format(epoch))
